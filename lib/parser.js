...
module.exports = function parser(identifier, string) {
  ...
  function parseOption(parent) {
    ...
    // Parses a singular value: string, number, bool, or a list of those.
    var parseValue = function () {
      ...
      if (peek().type === Token.Type.START_OPTION) {
        ...
        result = expect([Token.Type.STRING, Token.Type.WORD, Token.Type.NUMBER]).content
      }
      return result
    }
    ...
    if (peek().type == Token.Type.START_BLOCK) {
      ...
      parseBlock(function(token) {
        ...
        msgOption[token.content] = parseValue()
    })
    expectOptional(Token.Type.TERMINATOR)

    var existingOption = parent.getOption(key.content)
    if (existingOption) {
      var options = Array.isArray(existingOption) ? existingOption : [existingOption]
      options.push(msgOption)
      parent.addOption(key.content, options)
    } else {
      parent.addOption(key.content, msgOption)
    }

    // single line option
    } else {
      parent.addOption(key.content, parseValue())
      expect(Token.Type.TERMINATOR)
    }
  }
