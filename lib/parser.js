...
module.exports = function parser(string) {
  ...
  function parseEnum(parent) {
    ...
    parseBlock(function (token) {
      ...
      expect(Token.Type.OPERATOR, '=')
      var index = expect(Token.Type.WORD)
      if (isNaN(index.content)) {
        throw new ParseError(index, 'invalid enum "' + index.content + '"')
      }
      expect(Token.Type.TERMINATOR)
      enumeration.addValue(name, Number(index.content))
    })
  }
  ...
  function expect(type, value) {
    ...
    if (token.type != type || value && token.content != value) {
      throw new ParseError(token, 'expected token type ' + type)
    }
    return token
  }
