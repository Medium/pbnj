...
module.exports = function parser(string) {
  ...
  //     fieldType protoType name = index;
  //     fieldType protoType name = index [default = "value"];
  function parseField() {
    var type = expect(Token.Type.WORD).content
    var name = expect(Token.Type.WORD).content
    expect(Token.Type.OPERATOR, '=')
    var index = expect(Token.Type.WORD).content
    var field = new FieldDescriptor(type, name, index)
  ...
  // Parses a required field: required protoType name = index;
  ...
  // Parses an optional field: optional protoType name = index [default = 123];
  ...
  // Parses a repeated field: repeated protoType name = index [default = 123];
  ...
  // Gets the next token and throws an error if it doesn't match the desired type and value.
