 ...
 */
module.exports = function parser(filePath, string) {
  var tokens = tokenize(string)
  var proto = new ProtoDescriptor(filePath)
  ...
  while (tokens.length) {
    ...
    switch (token.content) {
      ...
      case 'message':
        ...
        break
      default:
        throw Error('Unexpected token in "' + filePath + '"; ' + token.content)
    }
  }
      ...
      switch (token.content) {
       ...
       case 'enum':
        ...
        case 'repeated':
          ...
          break
        default:
          throw new ParseError(filePath, token)
      }
    })
  }
  ...
  function parseField() {
    ...
    if (isNaN(tag.content)) {
      throw new ParseError(filePath, tag, 'invalid tag "' + tag.content + '"')
    }
  ...
  function parseEnum(parent) {
    ...
    parseBlock(function (token) {
      ...
      if (isNaN(number.content)) {
        throw new ParseError(filePath, index, 'invalid enum "' + number.content + '"')
      }
      expect(Token.Type.TERMINATOR)
  ...
  function expect(types) {
    ...
    if (!isOneOf(token, types)) {
      throw new ParseError(filePath, token, 'expected token to be one of ' + types)
    }
    return token
  }
...
function ParseError(name, token, msg) {
  ...
  this.message = 'Unexpected token in "' + name + '" at line:' + token.line +
      ', column:' + token.column + ', saw:' + token.content + (msg ? (', ' + msg) : '')
}
