...
Project.prototype.processFile = function (fileName, opt_followImports) {
  if (!this._protos[fileName]) {
    var fileContents = fs.readFileSync(path.join(this._basePath, fileName), 'utf8')
    var proto = this._protos[fileName] = parser(fileName, fileContents)
...
Project.prototype.loadTemplates = function (templateFolder) {
  ...
  soynode.compileTemplates(path.join(this._basePath, templateFolder), deferred.makeNodeResolver())
