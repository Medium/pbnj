...
Project.prototype.addProto = function (fileName, opt_followImports) {
  ...
  if (!this._protos[filePath]) {
    var fileContents = fs.readFileSync(filePath, 'utf8')
    var proto = this._protos[filePath] = parser(filePath, fileContents)
    if (opt_followImports) {
      var imports = proto.getImportNames()
      for (var i = 0; i < imports.length; i++) {
        var importPath = imports[i]
        this.addProto(importPath, true)
        proto.addImport(this._protos[importPath])
      }
    }
  }
  return this
